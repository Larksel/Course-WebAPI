...
namespace ReserveiAPI.Controllers
{
	...
	public class UserController : Controller
	{
		...
		public async Task<ActionResult> Login([FromBody] Login login)
		{
			...
			{
				_response.SetError();
				_response.Message = "Não foi possível realizar login";
				_response.Data = new
				{
					ErrorMessage = ex.Message,
					StackTrace = ex.StackTrace ?? "No stack trace available"
		...
		public async Task<ActionResult> Update([FromBody] UserDTO  userDTO)
		{
			...
			try
			{
				...
				{
					_response.SetNotFound();
					_response.Message = "Dados com conflito!";
					_response.Data = new 
					{
						errorId = "O usuário informado não existe!"
					};
					return NotFound(_response);
				}
			...
			catch (Exception ex)
			{
				...
				{
					ErrorMessage = ex.Message,
					StackTrace = ex.StackTrace ?? "No stack track available"
				};
				return StatusCode(StatusCodes.Status500InternalServerError, _response);
			}
		}
		...
		public async Task<ActionResult<UserDTO>> Delete(int id)
		{
			try
			{
				...
				if (userDTO is null)
				{
					...
					_response.Data = new 
					{
						errorId = "Usuário não encontrado!"
					};
					return NotFound(_response);
				}
